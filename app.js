...
function navigate(pid){
  ...
  if(pid==='padmin' && window._isAdmin) {
    setTimeout(()=>{
      _loadUsers();
      // Re-bind admin buttons sempre que entrar na página
...
function _setupLoginUI() {
  ...
  document.getElementById('first-btn')?.addEventListener('click', async () => {
    ...
    // 1. Buscar perfil pelo e-mail e verificar temp_password
    const { data: profileData, error: profileErr } = await _sb
      .from('profiles').select('id,temp_password,must_change_password').eq('email', email).single();

    if (profileErr || !profileData) {
      errEl.textContent = 'E-mail não encontrado. Verifique com o administrador.';
      errEl.style.display = ''; btn.disabled=false; btn.textContent='✓ Confirmar e Entrar'; return;
    }

    // 2. Verificar senha provisória
    if (!profileData.temp_password || profileData.temp_password !== tempPass) {
      errEl.textContent = 'Senha temporária incorreta. Verifique com o administrador.';
      errEl.style.display = ''; btn.disabled=false; btn.textContent='✓ Confirmar e Entrar'; return;
    }

    // 3. Tentar login com a senha temporária (caso seja primeiro acesso real)
    btn.textContent = '⏳ Autenticando...';
    let loginData, loginError;
    // Tentar com temp_password como senha do Supabase Auth
    const r1 = await _sb.auth.signInWithPassword({ email, password: tempPass });
    if (!r1.error) {
      loginData = r1.data;
    } else {
      // Caso a senha do Supabase seja diferente (reset pelo admin não funcionou),
      // não conseguimos fazer login — orientar o usuário
      errEl.textContent = 'Não foi possível autenticar. Peça ao administrador para verificar sua conta no painel.';
      errEl.style.display = ''; btn.disabled=false; btn.textContent='✓ Confirmar e Entrar'; return;
    }

    // 4. Atualizar para nova senha
    ...
    // 5. Limpar temp_password e marcar must_change_password = false
...
,
  'LAR':{
    ...
    sellin:{label:'Sell-in trend & 2026 target (YTD realized)',years:['2023','2024','2025'],vals:[12.3,13.1,25.3],target:21,ytd:8,ytdPct:'38%',max:30,ticks:[0,10,20,30]},
    sellinMix:{label:'Sell-in mix by category (2025)',center:'$25M',centerSub:'2025',items:[
      {n:'Herbicides',v:9.5,c:'#1F4E5F'},{n:'Fungicides',v:8.1,c:'#4A8FA0'},
      {n:'Insecticides',v:4.2,c:'#6B8E4E'},{n:'ST',v:2.1,c:'#A8B84B'},{n:'Other',v:1.4,c:'#C9B27C'}]},
    sellout:{label:'Sell-out — history, targets & 26/27 YTD',cats:['23/24','24/25','25/26','26/27 T','27/28 T','28/29 T'],vals:[8,12,16,22,35,52],histCount:3,ytdIdx:3,ytdVal:0.5,max:60,ticks:[0,20,40,60]},
    inventory:{pct:129,status:'ABOVE HEALTHY LINE',healthy:30,note:'Inventory april/26 ~19 MUSD  |  Returns 1.2 MUSD'},
    actionPlan:[
      '<b>Technical excellence and launches:</b> robust and segmented TSI plan, along with launches and strategic products.',
      '<b>Balance participation</b> in key branches, concentrating targets in medium-share units. Detailed planning in expansion regions.',
      '<b>Cooperative member segmentation:</b> expand reach among large growers.'
    ],
    soc:{label:'SOC - Crop Protection',items:[
      {n:'Outros',v:35.0,c:'#6B8E3F'},{n:'Syngenta',v:20.0,c:'#1F3B57'},
      {n:'UPL',v:15.0,c:'#C9A227'},{n:'Corteva',v:13.0,c:'#2A9D8F',hl:true},
      {n:'Bayer',v:10.0,c:'#B85C4A'},{n:'IHARA',v:7.0,c:'#3E7C6E'}],max:50},
  ...
  'COTRIJAL':{
    ...
    soc:{label:'SOC - Crop Protection',items:[
      {n:'Outros',v:30.0,c:'#6B8E3F'},{n:'Syngenta',v:22.0,c:'#1F3B57'},
      {n:'Corteva',v:20.0,c:'#2A9D8F',hl:true},{n:'UPL',v:14.0,c:'#C9A227'},
      {n:'Bayer',v:9.0,c:'#B85C4A'},{n:'IHARA',v:5.0,c:'#3E7C6E'}],max:50},
  ...
  'COPLACANA':{
    channel:'CHANNEL · COOPERATIVE · ESCALA',
    title:'Coplacana',logo:'COPLACANA',logoSub:'COOPERATIVA DOS PLANTADORES',
    sellin:{label:'Sell-in trend & 2026 target (YTD realized)',years:['2023','2024','2025'],vals:[32.7,37.8,38.9],target:45,ytd:18,ytdPct:'40%',max:55,ticks:[0,15,30,45]},
    sellinMix:{label:'Sell-in mix by category (2025)',center:'$39M',centerSub:'2025',items:[
      {n:'Herbicides',v:16,c:'#1F4E5F'},{n:'Fungicides',v:12,c:'#4A8FA0'},
      {n:'Insecticides',v:6,c:'#6B8E4E'},{n:'ST',v:3,c:'#A8B84B'},{n:'Other',v:2,c:'#C9B27C'}]},
    sellout:{label:'Sell-out — history, targets & 26/27 YTD',cats:['23/24','24/25','25/26','26/27 T','27/28 T','28/29 T'],vals:[22,32,40,48,57,68],histCount:3,ytdIdx:3,ytdVal:1,max:80,ticks:[0,20,40,60,80]},
    inventory:{pct:28,status:'HEALTHY LINE',healthy:30,note:'Inventory healthy (28% of sell-out)'},
    actionPlan:[
    ...
    soc:{label:'SOC - Crop Protection',items:[
      {n:'Outros',v:38.0,c:'#6B8E3F'},{n:'Syngenta',v:20.0,c:'#1F3B57'},
      {n:'Corteva',v:16.0,c:'#2A9D8F',hl:true},{n:'UPL',v:13.0,c:'#C9A227'},
      {n:'Bayer',v:8.0,c:'#B85C4A'},{n:'IHARA',v:5.0,c:'#3E7C6E'}],max:50},
  ...
  'COAMO':{
    ...
    sellin:{label:'Sell-in trend & 2026 target (YTD realized)',years:['2023','2024','2025'],vals:[34.2,27.0,24.3],target:34,ytd:11,ytdPct:'32%',max:40,ticks:[0,10,20,30,40]},
    sellinMix:{label:'Sell-in mix by category (2025)',center:'$24M',centerSub:'2025',items:[
      {n:'Herbicides',v:10,c:'#1F4E5F'},{n:'Fungicides',v:8,c:'#4A8FA0'},
      {n:'Insecticides',v:4,c:'#6B8E4E'},{n:'ST',v:1.5,c:'#A8B84B'},{n:'Other',v:0.8,c:'#C9B27C'}]},
    sellout:{label:'Sell-out — history, targets & 26/27 YTD',cats:['23/24','24/25','25/26','26/27 T','27/28 T','28/29 T'],vals:[18,22,25,35,58,85],histCount:3,ytdIdx:3,ytdVal:0.8,max:100,ticks:[0,25,50,75,100]},
    inventory:{pct:49,status:'ABOVE HEALTHY LINE',healthy:30,note:'Inventory april/26 ~11 MUSD'},
    actionPlan:[
      ...
      '<b>Alignment and close engagement</b> with the cooperative\'s technical team.'
    ],
    soc:{label:'SOC - Crop Protection',items:[
      {n:'Outros',v:42.0,c:'#6B8E3F'},{n:'Syngenta',v:22.0,c:'#1F3B57'},
      {n:'UPL',v:13.0,c:'#C9A227'},{n:'Corteva',v:10.0,c:'#2A9D8F',hl:true},
      {n:'Bayer',v:8.0,c:'#B85C4A'},{n:'IHARA',v:5.0,c:'#3E7C6E'}],max:50},
  ...
  'COOPERCITRUS':{
    channel:'CHANNEL · COOPERATIVE · MINDCOOP',
    title:'Coopercitrus',logo:'COOPERCITRUS',logoSub:'COOPERATIVA',
    sellin:{label:'Sell-in trend & 2026 target (YTD realized)',years:['2023','2024','2025'],vals:[29.1,27.5,25.6],target:33,ytd:12,ytdPct:'36%',max:40,ticks:[0,10,20,30,40]},
    sellinMix:{label:'Sell-in mix by category (2025)',center:'$26M',centerSub:'2025',items:[
      {n:'Herbicides',v:10,c:'#1F4E5F'},{n:'Fungicides',v:8,c:'#4A8FA0'},
      {n:'Insecticides',v:4,c:'#6B8E4E'},{n:'ST',v:2,c:'#A8B84B'},{n:'Other',v:2,c:'#C9B27C'}]},
    sellout:{label:'Sell-out — history, targets & 26/27 YTD',cats:['23/24','24/25','25/26','26/27 T','27/28 T','28/29 T'],vals:[18,22,25,27,29,32],histCount:3,ytdIdx:3,ytdVal:0.6,max:40,ticks:[0,10,20,30,40]},
    inventory:{pct:24,status:'HEALTHY LINE',healthy:30,note:'Inventory healthy (24% of sell-out)'},
    actionPlan:[
      ...
      '<b>Expansion in soybean, citrus, and coffee crops.</b>',
      '<b>Key products:</b> UTRISHA N ambassador program and demand generation actions for LINEAR and GAPPER.'
    ],
    soc:{label:'SOC - Crop Protection',items:[
      {n:'Outros',v:36.0,c:'#6B8E3F'},{n:'Syngenta',v:19.0,c:'#1F3B57'},
      {n:'UPL',v:15.0,c:'#C9A227'},{n:'Corteva',v:14.0,c:'#2A9D8F',hl:true},
      {n:'Bayer',v:9.0,c:'#B85C4A'},{n:'IHARA',v:7.0,c:'#3E7C6E'}],max:50},
...
async function _toggleTranslate(){
  ...
  if(!_isEnglish){
    ...
    // Salvar HTML original de todas as páginas e nav
    _origPageHTMLs = {};
    ...
    // Dicionário PT→EN extenso
    const D_TR = {
      // Menu e navegação
      'Principal':'Main','Gestão':'Management','Encontros':'Meetings','Admin':'Admin',
      'Visão Geral':'Overview','Dashboard Executivo':'Executive Dashboard',
      'Governança':'Governance','One page Coops':'One Page Coops',
      'Relacionamento Coops':'Coops Relationship','Timeline':'Timeline',
      'Cooperativas':'Cooperatives','Acompanhamento KPIs':'KPI Tracking',
      'Gestão de Encontros':'Meeting Management','Administração de Acessos':'Access Administration',
      // Títulos e subtítulos
      'Visão geral':'Overview','Cockpit Executivo':'Executive Cockpit',
      'Programas do ESCALA':'ESCALA Programs','A Jornada ESCALA':'The ESCALA Journey',
      'Informações Relevantes — Destaques':'Relevant Information — Highlights',
      'Últimos Destaques':'Latest Highlights','Decisões Recentes':'Recent Decisions',
      'Riscos e Alertas':'Risks & Alerts','Calendário de Processos 2026':'2026 Process Calendar',
      'Plano de Aceleração das Cooperativas':'Cooperative Acceleration Plan',
      'Time Interno':'Internal Team','Mapeamento de Temas':'Theme Mapping',
      'Acompanhamento dos Rituais':'Ritual Tracking','Rituais de Gestão':'Management Rituals',
      'Marcos do Projeto':'Project Milestones','Visualização da Timeline':'Timeline View',
      'Equipe Responsável por Cooperativa':'Team by Cooperative',
      // Campos comuns
      'Cooperativa':'Cooperative','Cooperativas':'Cooperatives','Nome':'Name',
      'Data':'Date','Detalhe':'Detail','Status':'Status','Responsável':'Owner',
      'Responsáveis':'Owners','Prazo':'Deadline','Fase':'Phase','Objetivo':'Objective',
      'Local':'Location','Observações':'Notes','Categoria':'Category','Empresa':'Company',
      'Perfil':'Profile','E-mail':'Email','Telefone':'Phone','Ação':'Action','Ações':'Actions',
      'Ritual':'Ritual','Diretoria':'Board','Plano':'Plan','Item':'Item',
      'Segm.':'Segm.','Resp.':'Owner','Formato':'Format','Enviado para KAM':'Sent to KAM',
      'Recebido pelo KAM':'Received by KAM','Radar':'Radar',
      // Entregáveis e fases
      'Entregáveis Estratégicos':'Strategic Deliverables','Entregável':'Deliverable',
      'Entregáveis':'Deliverables','Fases':'Phases','Informações Gerais':'General Information',
      'Objetivo do Encontro':'Meeting Objective','Público-Alvo':'Target Audience',
      'Data Prevista':'Expected Date','Plano de Ação':'Action Plan',
      'Aprendizados':'Learnings','Decisões':'Decisions',
      'Validação de tema':'Theme validation','Início RSVP':'RSVP start',
      'Recebimento do tema de projeto aplicado':'Receipt of applied project theme',
      'Recebimento do projeto final':'Receipt of final project',
      'Data definida':'Date defined','Reunião com KAM':'Meeting with KAM',
      'Reunião com Coop':'Meeting with Coop','Lista de participantes validada':'Validated participant list',
      'Treinamento executado':'Training executed','Captura de NPS':'NPS capture',
      // Status
      'Não iniciado':'Not started','Em progresso':'In progress','Em prep.':'Preparing',
      'Concluído':'Completed','Atrasado':'Delayed','Em atenção':'Attention needed',
      'No plano':'On track','Crítico':'Critical','Ativo':'Active','Bloqueado':'Blocked',
      'Pendente':'Pending','—':'—',
      // Botões e ações
      'Exportar PDF':'Export PDF','Salvar':'Save','Alterar senha':'Change password',
      'Sair':'Log out','Adicionar':'Add','Excluir':'Delete','Editar':'Edit',
      'Confirmar':'Confirm','Cancelar':'Cancel','Convidar Usuário':'Invite User',
      '+ Coop':'+ Coop','+ Linha':'+ Line','+ Marco':'+ Milestone','+ Cooperativa':'+ Cooperative',
      'Todas':'All','Todos os perfis':'All profiles','Todos os status':'All status',
      'Buscar nome ou e-mail...':'Search name or email...',
      // Colunas tabelas
      'Cooperativa':'Cooperative','KAM / DSM':'KAM / DSM','E-mail KAM':'KAM Email',
      'Tel. KAM':'KAM Phone','E-mail RSL':'RSL Email','Tel. RSL':'RSL Phone',
      'E-mail RML':'RML Email','Tel. RML':'RML Phone','Relacionamento Atual':'Current Relationship',
      'Criado em':'Created on','Último Acesso':'Last Access','Ações':'Actions',
      'Total de Usuários':'Total Users','Ativos':'Active','Bloqueados':'Blocked','Pendentes':'Pending',
      // Governança
      'Rituais de Gestão':'Management Rituals','Próximos Passos e Responsáveis':'Next Steps & Owners',
      'Fóruns de Execução':'Execution Forums','Reuniões com KAMs/RSLs':'Meetings with KAMs/RSLs',
      // One Page
      'SELL-IN':'SELL-IN','SELL-OUT':'SELL-OUT','INVENTORY':'INVENTORY',
      'ACTION PLAN':'ACTION PLAN','KEY TAKEAWAY':'KEY TAKEAWAY','SELL-IN MIX':'SELL-IN MIX',
      'Sell-in mix by category (2025)':'Sell-in mix by category (2025)',
      'ABOVE HEALTHY LINE':'ABOVE HEALTHY LINE','HEALTHY LINE':'HEALTHY LINE',
      // Relacionamento
      'Ótimo':'Excellent','Bom':'Good','Neutro':'Neutral','Ruim':'Poor','Crítico':'Critical',
      // Topbar
      'Atualizado hoje':'Updated today','Última atualização':'Last update',
      'Atualizado':'Updated',
      // SOC
      'Outros':'Others',
      // Anotações comuns
      'Sem preocupações urgentes':'No urgent concerns',
      'Não iniciado':'Not started',
    };

    // Aplicar traduções em todos os nós de texto
    function translateNode(el){
      const walker = document.createTreeWalker(el, NodeFilter.SHOW_TEXT, {
        acceptNode: n => {
          const p = n.parentElement;
          if(!p) return NodeFilter.FILTER_REJECT;
          if(['SCRIPT','STYLE','OPTION','INPUT','TEXTAREA'].includes(p.tagName)) return NodeFilter.FILTER_REJECT;
      ...
      nodes.forEach(n=>{
        ...
        if(D_TR[trimmed]) {
          n.textContent = t.replace(trimmed, D_TR[trimmed]);
        } else {
          // Tentar tradução palavra a palavra para textos não encontrados
          let result = t;
          Object.entries(D_TR).forEach(([pt,en])=>{
            if(pt.length > 3 && result.includes(pt)) result = result.replace(new RegExp(pt.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'g'), en);
          });
          if(result !== t) n.textContent = result;
        }
      });
    }

    document.querySelectorAll('.page, nav').forEach(translateNode);

...
async function _toggleTranslate(){
  ...
  if(!_isEnglish){
    ...
    // Guardar HTMLs originais de TODAS as páginas e menu
    _origPageHTMLs = {};
    ...
    // Coletar TODOS os nós de texto do cockpit (exceto scripts/styles)
    const allTexts = new Map(); // texto original → [nodes]
    const sections = [...document.querySelectorAll('.page, nav .lbl, .slabel')];
    
    // Nomes que NÃO devem ser traduzidos
    const noTranslate = new Set([
      'MINDCOOP','INFOCOOP','INTECOOP','ELITECOOP','GOCOOP','ESCALA','ESCALA 2026',
      'CORTEVA','ZMP','KAM','RSL','RML','DSM','NPS','SOC','PINC','PAC',
      'RSVP','PDF','PT','EN','Supabase','Netlify',
    ]);
    // Padrão: nomes de pessoas (TODO CAPS com espaço), coops, e-mails, números
    const skipPattern = /^[A-Z]{2,}(\s[A-Z]{2,})+$|@|\d{2}\/\d{2}|\d+%|R\$|\$|^[\d\s\-\+\.]+$/;

    function collectTexts(el){
      const walker = document.createTreeWalker(el, NodeFilter.SHOW_TEXT, {
        acceptNode: n => {
          const p = n.parentElement;
          if(!p || ['SCRIPT','STYLE','OPTION'].includes(p.tagName)) return NodeFilter.FILTER_REJECT;
          const t = n.textContent.trim();
          if(!t || t.length < 3) return NodeFilter.FILTER_SKIP;
          if(noTranslate.has(t)) return NodeFilter.FILTER_SKIP;
          if(skipPattern.test(t)) return NodeFilter.FILTER_SKIP;
      ...
      while(node = walker.nextNode()){
        ...
        if(!allTexts.has(t)) allTexts.set(t,[]);
        allTexts.get(t).push(node);
      }
    }
    document.querySelectorAll('.page, nav').forEach(collectTexts);

    const uniqueTexts = [...allTexts.keys()].filter(t=>t.length>=3 && t.length<=500);
        ...
        const resp = await fetch('https://api.anthropic.com/v1/messages',{
          ...
          body:JSON.stringify({
            ...
            messages:[{role:'user',content:
              'You are a professional translator. Translate the following Brazilian Portuguese texts to formal business English.\n\nRULES:\n- Return ONLY a valid JSON object mapping each original text to its English translation\n- Keep proper nouns (cooperative names, person names, program names like MINDCOOP/ESCALA/GOCOOP, company names) unchanged\n- Keep acronyms (KAM, RSL, RML, SOC, NPS, etc.) unchanged\n- Keep numbers, dates, percentages, currencies unchanged\n- Translate all other words naturally and professionally\n- No markdown, no backticks, just the JSON\n\nTexts to translate:\n'+JSON.stringify(batch)
            }]
          })
        });
    ...
    // Aplicar traduções nos nós de texto
    allTexts.forEach((nodes,orig)=>{
      const tr = translations[orig];
      if(tr && tr!==orig) nodes.forEach(n=>{ n.textContent = n.textContent.replace(orig,tr); });
    });

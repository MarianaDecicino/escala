...
async function _confirmReset(userId) {
  ...
  // Salvar senha provisória no perfil + marcar must_change_password
  const { error } = await _sb.from('profiles')
    .update({ temp_password: pwd, must_change_password: true })
    .eq('id', userId);
  if(error) {
  ...
  // Mostrar instrução clara para o admin
  setTimeout(()=>{
    alert('✓ Senha provisória definida!\n\nOriente a pessoa a:\n1. Entrar no site\n2. Clicar em "Primeiro Acesso"\n3. Usar o e-mail dela + a senha provisória: "'+pwd+'"\n4. Criar uma nova senha pessoal');
  }, 100);
  showToast();
}

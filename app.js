    ...
    ['MINDCOOP','INFOCOOP','INTECOOP','ELITECOOP','GOCOOP'].forEach(k=>{
      ...
      if(D0.encontros[k] && D0.encontros[k].entregaveis && D0.encontros[k].entregaveis.length > 0 && D0.encontros[k].entregaveis[0].coop) {
        if(!D.encontros[k].entregaveis || !D.encontros[k].entregaveis.length || !D.encontros[k].entregaveis[0].coop) {
          ...
          // Deduplicar — remover entradas com mesma coop+ent
          const seen = new Set();
          D.encontros[k].entregaveis = D.encontros[k].entregaveis.filter(e=>{
            const key = (e.coop||'')+'|'+(e.ent||'');
...
// ═══════════ RENDER ALL ═══════════
function _fixMobileLayout(){
  if(window.innerWidth > 768) return;
  // Colapsar todos os grids inline para 1 coluna no mobile
  document.querySelectorAll('[style*="grid-template-columns"]').forEach(el=>{
    el.style.gridTemplateColumns = '1fr';
  });
  // Colapsar display:grid inline
  document.querySelectorAll('[style*="display:grid"],[style*="display: grid"]').forEach(el=>{
    el.style.gridTemplateColumns = '1fr';
  });
  // Garantir que flexbox inline não transborde
  document.querySelectorAll('[style*="display:flex"],[style*="display: flex"]').forEach(el=>{
    if(!el.style.flexWrap) el.style.flexWrap = 'wrap';
  });
}
function renderAll(){
  try{_initLastSavedBadges();}catch(e){console.warn('badges',e);}
  try{setTimeout(_fixMobileLayout, 200);}catch(e){}
  // Mostrar última atualização global na topbar ao carregar
  try{
    if(D.lastSaved){
      const times=Object.values(D.lastSaved).filter(Boolean);
      if(times.length){
        // Pegar a mais recente
        const latest=times.sort().pop();
        const el=document.getElementById('topbar-lastsaved');
        const txt=document.getElementById('topbar-lastsaved-txt');
        if(el&&txt){txt.textContent='Atualizado: '+latest;el.style.display='flex';}
      }
    }
  }catch(e){}
  ...
  if(p.nome === 'INFOCOOP') {
    ...
    function _renderAtuTable(){
      ...
      tableDiv.onclick = function(e){
        const td = e.target.closest('td[data-info-field]');
        if(!td) return;
        const field = td.dataset.infoField;
        const idx2 = parseInt(td.dataset.infoIdx);
        ...
        if(tag){tag.className='tag '+(cls[next]||'tag-gray');tag.textContent=labels[next]||'Não iniciado';}
        saveD(true);
      };
    }
...
function _toggleMobMenu(){
  ...
  if(isOpen){ nav.classList.remove('mob-open'); if(overlay) overlay.classList.remove('show'); }
  else { nav.classList.add('mob-open'); if(overlay) overlay.classList.add('show'); }
}
...
function navigate(pid){
  ...
  const _pg=document.getElementById('page-'+pid);
  if(_pg) _pg.classList.add('active');
  else{console.error('page not found:',pid);return;}
  try{document.getElementById('bc-text').textContent=BC[pid]||pid;}catch(e){}
  document.querySelectorAll('.ni').forEach(n=>{if(n.dataset.page===pid) n.classList.add('active');});
  try{if(['pmind','pinfo','pinte','pelite','pgo'].includes(pid)) renderProgramaPage(pid);}catch(e){}
  try{if(pid==='psellinout') renderSellOut();}catch(e){}
  if(pid==='padmin' && window._isAdmin){
    setTimeout(()=>{
      try{_loadUsers();}catch(e){}
      try{document.getElementById('adm-invite-btn')?.addEventListener('click',()=>_openModal(null));}catch(e){}
      try{document.getElementById('adm-modal-close')?.addEventListener('click',_closeModal);}catch(e){}
      try{document.getElementById('adm-modal-cancel')?.addEventListener('click',_closeModal);}catch(e){}
      try{document.getElementById('adm-modal-save')?.addEventListener('click',_saveUser);}catch(e){}
      ['adm-search','adm-fperfil','adm-fstatus'].forEach(id=>{
        try{document.getElementById(id)?.addEventListener('input',_filterUsers);}catch(e){}
        try{document.getElementById(id)?.addEventListener('change',_filterUsers);}catch(e){}
      });
    },100);
  }
  try{if(pid==='p1'){renderTimeline();renderProgCards();}}catch(e){}
  try{if(pid==='p2') renderDashboard();}catch(e){}
  try{if(pid==='p3') renderKpiAreas();}catch(e){}
  try{if(pid==='ponepage') renderOnePage();}catch(e){}
  try{if(pid==='p4') renderEncontros();}catch(e){}
  try{if(pid==='p5'){renderCoops();renderTimeInterno();}}catch(e){}
  try{if(pid==='prelac') renderRelac();}catch(e){}
  try{if(pid==='ppac') renderPac();}catch(e){}
  try{if(pid==='pgov'){renderRituais();renderRitualMatrix();renderGovCoops();renderTemas();}}catch(e){}
  try{if(pid==='ppinc') renderPinc();}catch(e){}
  try{if(pid==='ptimeline'){renderMarcos();renderTimelineVisual();}}catch(e){}
  try{if(pid==='pfotos') renderFotos();}catch(e){}
  try{setTimeout(_fixMobileLayout,300);}catch(e){}
}
...
function saveD(silent=false){
  ...
  if(_activePage){
    ...
    const _now = new Date();
    const _ts = _now.toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'});
    D.lastSaved[_pid] = _ts;
    _renderLastSaved(_pid);
    // Atualizar topbar
    _updateTopbarTime(_now);
  }
  st=setTimeout(async ()=>{
...
function _updateTopbarTime(date){
  const el = document.getElementById('topbar-lastsaved');
  const txt = document.getElementById('topbar-lastsaved-txt');
  if(!el||!txt) return;
  ...
  txt.textContent = isToday ? 'Atualizado hoje, '+time : 'Atualizado '+now.toLocaleDateString('pt-BR')+' '+time;
  el.style.display = 'flex';
}
function _renderLastSaved(pid){
  const el = document.getElementById('last-saved-'+pid);
  if(el && D.lastSaved && D.lastSaved[pid]){
    el.textContent = '🕐 Última atualização: '+D.lastSaved[pid];
...
function _setupLoginUI() {
  ...
  document.getElementById('first-btn')?.addEventListener('click', async () => {
    ...
    // 1. Tentar login direto com a senha temporária
    const { data: loginData, error: loginErr } = await _sb.auth.signInWithPassword({ email, password: tempPass });

    if (loginErr) {
      errEl.textContent = 'E-mail ou senha temporária incorretos. Verifique com o administrador.';
      errEl.style.display = '';
    ...
    // 3. Limpar must_change_password e temp_password
    await _sb.from('profiles').update({ must_change_password: false, temp_password: null }).eq('id', loginData.user.id);
...
async function _inviteUserFallback(email, nome, empresa, perfil) {
  ...
  if (error) {
    ...
    if (msg.includes('already registered') || msg.includes('already been registered') || msg.includes('User already registered')) {
      ...
      return { data: null, error: new Error('Este e-mail já existe. Exclua o usuário no admin e tente novamente.') };
    }
    if (msg.includes('rate limit') || msg.includes('email rate')) {
      return { data: null, error: new Error('Limite atingido. Aguarde alguns minutos.') };
    }
    return { data: null, error: new Error(msg) };
  }
  
  // Usuário criado — salvar perfil e confirmar e-mail via SQL direto
  if (data && data.user) {
    // Confirmar e-mail automaticamente (evita necessidade de confirmação)
    try{ await _sb.rpc('confirm_user_email', { user_id: data.user.id }); }catch(e){}
...
async function _confirmReset(userId) {
  ...
  // 1. Buscar perfil atual para preservar dados
  const { data: profile, error: profErr } = await _sb
    .from('profiles').select('*').eq('id', userId).single();
  if(profErr || !profile){
    err.textContent='Erro ao buscar perfil: '+(profErr?.message||'não encontrado');
    err.style.display=''; btn.disabled=false; btn.textContent='✓ Redefinir senha'; return;
  }

  btn.textContent='⏳ Excluindo conta antiga...';
  // 2. Deletar perfil (cascade deleta o auth.user via FK ou trigger)
  const { error: delErr } = await _sb.from('profiles').delete().eq('id', userId);
  if(delErr){
    err.textContent='Erro ao resetar: '+delErr.message+'. Tente novamente.';
    err.style.display=''; btn.disabled=false; btn.textContent='✓ Redefinir senha'; return;
  }

  btn.textContent='⏳ Recriando com nova senha...';
  // 3. Recriar usuário com a senha provisória
  const { data: newUser, error: signUpErr } = await _sb.auth.signUp({
    email: profile.email,
    password: pwd,
    options: { data: { nome: profile.nome, empresa: profile.empresa, perfil: profile.perfil } }
  });
  if(signUpErr || !newUser?.user){
    err.textContent='Erro ao recriar conta: '+(signUpErr?.message||'tente novamente');
    err.style.display=''; btn.disabled=false; btn.textContent='✓ Redefinir senha'; return;
  }

  // 4. Recriar perfil com os mesmos dados + must_change_password = true
  await _sb.from('profiles').upsert({
    id: newUser.user.id,
    email: profile.email,
    nome: profile.nome,
  ...
  showToast();
  setTimeout(()=>{
    alert('✓ Senha provisória definida com sucesso!\n\nOriente a pessoa:\n1. Acessar o site\n2. Clicar em "Primeiro Acesso"\n3. Informar o e-mail: '+profile.email+'\n4. Informar a senha provisória: '+pwd+'\n5. Criar uma nova senha pessoal');
  },300);
}
...
const ONEPAGE_DATA={
  'COAMO':{
    channel:'CHANNEL · COOPERATIVE · MINDCOOP',
    title:'Coamo',logo:'COAMO',logoSub:'AGROINDUSTRIAL COOPERATIVA',
    sellin:{label:'Sell-in trend & 2026 target (YTD realized)',years:['2023','2024','2025'],vals:[45.4,20.6,24.0],target:34,ytd:23,ytdPct:'68%',max:70,ticks:[0,20,40,60]},
    sellinMix:{label:'Sell-in mix by category (2025)',center:'$24M',centerSub:'2025',items:[
      {n:'Fungicides (10.7)',v:10.7,c:'#4A8FA0'},{n:'Herbicides (5.2)',v:5.2,c:'#1F4E5F'},
      {n:'Insecticides (5.2)',v:5.2,c:'#6B8E4E'},{n:'Pasture & IVM (2.1)',v:2.1,c:'#A8B84B'},
      {n:'ST (0.7)',v:0.7,c:'#C9B27C'}]},
    sellout:{label:'Sell-out — history, targets & 26/27 YTD',cats:['23/24','24/25','25/26','26/27 T','27/28 T','28/29 T'],vals:[67,37,58,70,78,85],histCount:3,ytdIdx:3,ytdVal:2,max:100,ticks:[0,25,50,75,100]},
    inventory:{pct:50,status:'ABOVE HEALTHY LINE',healthy:30,note:'Inventory july/26  29MUSD   |   Sellout 25/26  58MUSD'},
    mix:{label:'Mix de produto — COAMO 2025',targetLine:67,note:'Estratégico + Lançamento 41%',items:[
      {label:'Lançamento',v:12,c:'#C9A227'},{label:'Estratégico',v:16,c:'#4A8FA0'},
      {label:'Regular',v:25,c:'#6B8E4E'},{label:'Commodity',v:47,c:'#8B9BAD'}]},
    actionPlan:[
      '<b>Accelerate sell-out growth</b>, focusing on new branches (south PR), strategic cooperative members (80/20), and key products (Gapper, Vicroya, Aproach Premium, Exalt, Elevore e TSI).',
      '<b>Ensure disciplined execution</b>, through campaigns, demand generation, training, and performance indicator tracking.',
      '<b>Alignment</b> and close engagement with the cooperative\'s <b>technical team.</b>'
    ],
    soc:{label:'SOC - Crop Protection (Ranking by Position)',ranking:true,items:[
      {n:'Bayer',place:'1st place',v:100,c:'#B85C4A'},
      {n:'BASF',place:'2nd place',v:80,c:'#1F3B57'},
      {n:'Syngenta',place:'3rd place',v:60,c:'#4A8FA0'},
      {n:'UPL',place:'4th place',v:40,c:'#C9A227'},
      {n:'Corteva',place:'5th place',v:25,c:'#2A9D8F',hl:true}],max:100},
    takeaway:'Sell-in +17% in 2025 (-27% CAGR from 2023); sell-out targeted +131% to $85M by 28/29; inventory high (49% of sell-out, >30% healthy line).<br>Challenge: 2026 sell-in target $34M is +40% vs 2025 — stock must sell down first while stock is high.<br><small>Note: Inventory in april/26 was~ 11 MUSD.</small>'
  },
  'COPLACANA':{
    channel:'CHANNEL · COOPERATIVE · ESCALA MINDCOOP',
    title:'Coplacana',logo:'COPLACANA',logoSub:'COOPERATIVA DOS PLANTADORES',
    sellin:{label:'Sell-in trend & 2026 target (YTD realized)',years:['2023','2024','2025'],vals:[30.9,35.3,42.0],target:45,ytd:10,ytdPct:'23%',max:60,ticks:[0,20,40,60]},
    sellinMix:{label:'Sell-in mix by category (2025)',center:'$42M',centerSub:'2025',items:[
      {n:'Sugarcane (36.4)',v:36.4,c:'#1F4E5F'},{n:'Fungicides (2.8)',v:2.8,c:'#4A8FA0'},
      {n:'Herbicides (1.1)',v:1.1,c:'#6B8E4E'},{n:'Insecticides (1.0)',v:1.0,c:'#A8B84B'},
      {n:'Biologicals (0.4)',v:0.4,c:'#C9B27C'},{n:'Other (0.2)',v:0.2,c:'#555'}]},
    sellout:{label:'Sell-out — history, targets & 26/27 YTD',cats:['23/24','24/25','25/26','26/27 T','27/28 T','28/29 T'],vals:[41,40,41,45,52,68],histCount:3,ytdIdx:3,ytdVal:10,max:80,ticks:[0,20,40,60,80]},
    inventory:{pct:27,status:'WITHIN HEALTHY LINE',healthy:30,note:'Inventory july/26  11MUSD   |   Sellout 25/26  41MUSD'},
    mix:{label:'Mix de produto — COPLACANA 2025',targetLine:67,note:'Estratégico + Lançamento 65%',items:[
      {label:'Lançamento',v:14,c:'#C9A227'},{label:'Estratégico',v:21,c:'#4A8FA0'},
      {label:'Regular',v:16,c:'#6B8E4E'},{label:'Commodity',v:49,c:'#8B9BAD'}]},
    actionPlan:[
      '<b>Train and enable the commercial team</b> — key products, UTRISHA N, cereals, and sugarcane.',
      'Detailed planning for <b>expansion regions</b>, with emphasis on the <b>Midwest.</b>',
    ...
    soc:{label:'SOC - Crop Protection',items:[
      {n:'Outros',v:28.7,c:'#6B8E3F'},{n:'Bayer',v:24.4,c:'#B85C4A'},
      {n:'FMC',v:17.6,c:'#1F3B57'},{n:'Corteva',v:12.4,c:'#2A9D8F',hl:true},
      {n:'IHARA',v:9.2,c:'#3E7C6E'},{n:'Syngenta',v:7.7,c:'#4A8FA0'}],max:35},
    takeaway:'Sell-in +19% in 2025 (+17% CAGR from 2023); sell-out targeted +70% to $68M by 28/29; inventory healthy (28% of sell-out).<br>Challenge: 2026 sell-in target $45M is ~flat vs 2025, depends on sell-out pace.'
  },
  'LAR':{
    channel:'CHANNEL · COOPERATIVE · MINDCOOP',
    title:'Lar',logo:'LAR',logoSub:'COOPERATIVA AGROINDUSTRIAL',
    sellin:{label:'Sell-in trend & 2026 target (YTD realized)',years:['2023','2024','2025'],vals:[23.5,13.1,25.4],target:21,ytd:7,ytdPct:'33%',max:30,ticks:[0,10,20,30]},
    sellinMix:{label:'Sell-in mix by category (2025)',center:'$25M',centerSub:'2025',items:[
      {n:'Fungicides (11.2)',v:11.2,c:'#4A8FA0'},{n:'Herbicides (10.6)',v:10.6,c:'#1F4E5F'},
      {n:'Biologicals (2.2)',v:2.2,c:'#6B8E4E'},{n:'Insecticides (1.0)',v:1.0,c:'#A8B84B'},
      {n:'Pasture & IVM (0.3)',v:0.3,c:'#C9B27C'},{n:'Other (0.1)',v:0.1,c:'#555'}]},
    sellout:{label:'Sell-out — history, targets & 26/27 YTD',cats:['23/24','24/25','25/26','26/27 T','27/28 T','28/29 T'],vals:[27,16,23,36,44,52],histCount:3,ytdIdx:3,ytdVal:4,max:60,ticks:[0,20,40,60]},
    inventory:{pct:96,status:'ABOVE HEALTHY LINE',healthy:30,note:'Inventory july/26  22MUSD   |   Sellout 25/26  23MUSD'},
    mix:{label:'Mix de produto — LAR 2025',targetLine:67,note:'Estratégico + Lançamento 44%',items:[
      {label:'Lançamento',v:11,c:'#C9A227'},{label:'Estratégico',v:33,c:'#4A8FA0'},
      {label:'Regular',v:11,c:'#6B8E4E'},{label:'Commodity',v:45,c:'#8B9BAD'}]},
    actionPlan:[
      '<b>Technical excellence and launches:</b> robust and segmented TSI plan, along with launches and strategic products.',
      '<b>Balance participation in key branches</b>, concentrating targets in <b>medium-share units.</b> Detailed planning in <b>expansion regions.</b>',
      '<b>Cooperative member segmentation:</b> expand reach among large growers.'
    ],
    soc:{label:'SOC - Crop Protection',items:[
      {n:'Outros',v:54.2,c:'#6B8E3F'},{n:'Syngenta',v:16.7,c:'#4A8FA0'},
      {n:'BASF',v:10.6,c:'#1F3B57'},{n:'Corteva',v:7.2,c:'#2A9D8F',hl:true},
      {n:'UPL',v:5.8,c:'#C9A227'},{n:'Bayer',v:5.5,c:'#B85C4A'}],max:60},
    takeaway:'Sell-in +94% in 2025 (+4% CAGR from 2023); sell-out targeted +230% to $52M by 28/29; inventory high (129% of sell-out, >30% healthy line).<br>Challenge: 2026 sell-in target $21M is -16% vs 2025 — a reset to clear inventory while stock is high.<br><small>Note: Inventory in april/26 was~ 19 MUSD and we had 1.2 MUSD of returns.</small>'
  },
  'COOPERCITRUS':{
    channel:'CHANNEL · COOPERATIVE · MINDCOOP',
    title:'Coopercitrus',logo:'COOPERCITRUS',logoSub:'COOPERATIVA DE PRODUTORES RURAIS',
    sellin:{label:'Sell-in trend & 2026 target (YTD realized)',years:['2023','2024','2025'],vals:[25.4,28.9,25.5],target:33,ytd:0,ytdPct:'—',max:40,ticks:[0,10,20,30,40]},
    sellinMix:{label:'Sell-in mix by category (2025)',center:'$26M',centerSub:'2025',items:[
      {n:'Sugarcane (16.0)',v:16.0,c:'#1F4E5F'},{n:'F&V, Coffee & Citrus (3.0)',v:3.0,c:'#4A8FA0'},
      {n:'Insecticides (2.0)',v:2.0,c:'#6B8E4E'},{n:'ST (1.8)',v:1.8,c:'#A8B84B'},
      {n:'Herbicides (1.3)',v:1.3,c:'#C9B27C'},{n:'Other (1.4)',v:1.4,c:'#555'}]},
    sellout:{label:'Sell-out — history, targets & 26/27 YTD',cats:['23/24','24/25','25/26','26/27 T','27/28 T','28/29 T'],vals:[30,30,28,29,30,32],histCount:3,ytdIdx:3,ytdVal:2,max:40,ticks:[0,10,20,30,40]},
    inventory:{pct:36,status:'ABOVE HEALTHY LINE',healthy:30,note:'Inventory july/26  10MUSD   |   Sellout 25/26  28MUSD'},
    mix:{label:'Mix de produto — COOPERCITRUS 2025',targetLine:67,note:'Estratégico + Lançamento 74%',items:[
      {label:'Lançamento',v:21,c:'#C9A227'},{label:'Estratégico',v:53,c:'#4A8FA0'},
      {label:'Regular',v:18,c:'#6B8E4E'},{label:'Commodity',v:8,c:'#8B9BAD'}]},
    actionPlan:[
      '<b>Selection of priority branches</b>, segmentation of <b>cooperative members (Top 20)</b>, definition of products, targets, and action plan by branch.',
      '<b>Expansion</b> in soybean, citrus, and coffee crops.',
      '<b>Key products: UTRISHA N</b> ambassador program and demand generation actions for <b>LINEAR and GAPPER.</b>'
    ],
    soc:{label:'SOC - Crop Protection',items:[
      {n:'Outros',v:32.9,c:'#6B8E3F'},{n:'Bayer',v:22.6,c:'#B85C4A'},
      {n:'Syngenta',v:15.0,c:'#4A8FA0'},{n:'IHARA',v:12.5,c:'#3E7C6E'},
      {n:'FMC',v:10.0,c:'#1F3B57'},{n:'Corteva',v:7.0,c:'#2A9D8F',hl:true}],max:40},
    takeaway:'Sell-in -12% in 2025 (+0% CAGR from 2023); sell-out targeted +7% to $32M by 28/29; inventory healthy (24% of sell-out).<br>Challenge: 2026 sell-in target $33M is +28% vs 2025 — stock must sell down first.'
  },
  'COMIGO':{
    channel:'CHANNEL · COOPERATIVE · MINDCOOP',
    title:'Comigo',logo:'COMIGO',logoSub:'COOPERATIVA MISTA DOS PRODUTORES RURAIS',
    sellin:{label:'Sell-in trend & 2026 target (YTD realized)',years:['2023','2024','2025'],vals:[37.5,31.2,23.8],target:20,ytd:14,ytdPct:'67%',max:45,ticks:[0,15,30,45]},
    sellinMix:{label:'Sell-in mix by category (2025)',center:'$24M',centerSub:'2025',items:[
      {n:'Fungicides (12.4)',v:12.4,c:'#4A8FA0'},{n:'Herbicides (4.2)',v:4.2,c:'#1F4E5F'},
      {n:'Insecticides (3.5)',v:3.5,c:'#6B8E4E'},{n:'Pasture & IVM (3.1)',v:3.1,c:'#A8B84B'},
      {n:'Rice (0.5)',v:0.5,c:'#C9B27C'},{n:'Other (0.1)',v:0.1,c:'#555'}]},
    sellout:{label:'Sell-out — history, targets & 26/27 YTD',cats:['23/24','24/25','25/26','26/27 T','27/28 T','28/29 T'],vals:[27,21,20,24,27,32],histCount:3,ytdIdx:3,ytdVal:2,max:40,ticks:[0,10,20,30,40]},
    inventory:{pct:255,status:'ABOVE HEALTHY LINE',healthy:30,note:'Inv 50MUSD  |  Sell-out 25/26  20MUSD'},
    mix:{label:'Mix de produto — COMIGO 2025',targetLine:67,note:'Estratégico + Lançamento 27%',items:[
      {label:'Lançamento',v:15,c:'#C9A227'},{label:'Estratégico',v:13,c:'#4A8FA0'},
      {label:'Regular',v:14,c:'#6B8E4E'},{label:'Commodity',v:59,c:'#8B9BAD'}]},
    actionPlan:[
      '<b>Commercial Activation & Sell-Out:</b> Drive sales through barter/cross-sell offers (Ling CP + Seeds with cashback), performance bonuses for Sales Consultants, and relationship-building initiatives.',
      '<b>Technical Support & Demand Generation:</b> Deploy 410 Crop Protection demand-generation fields (150 soybean/corn + 260 Brevant winter corn/safrinha), field days, results tours, region- and crop-specific technical recommendations, and personalized content shared via WhatsApp.',
      '<b>Marketing, Inventory Management & Governance:</b> Strengthen brand and campaigns via outdoor, radio, social media, and grower testimonials; optimize stock through DSM (prioritizing near-expiry and high-demand products); and maintain monthly governance forums to track KPIs (led by Luiz Oliveira, Wheverton Cabral, Carlos Teoro, and Carlos Melo).'
    ],
    soc:{label:'SOC - Crop Protection (Ranking by Position)',ranking:true,items:[
      {n:'Player 1',place:'1st place',v:100,c:'#6B8E3F'},
      {n:'Corteva',place:'2nd place (15%)',v:80,c:'#2A9D8F',hl:true},
      {n:'Player 3',place:'3rd place',v:55,c:'#1F3B57'},
      {n:'Player 4',place:'4th place',v:35,c:'#C9A227'},
      {n:'Player 5',place:'5th place',v:20,c:'#B85C4A'}],max:100},
    takeaway:'Sell-in 2025 US$23.8M (-24% vs 2024); sell-out 25/26 US$19.8M, targeted to US$32.0M by 28/29; inventory 255% of sell-out (>30%, above healthy line).<br>Challenge: 2026 sell-in target US$20.2M vs 2025 US$23.8M (-15%); 2026 YTD invoiced+orders US$13.5M.<br><small>Note: Values in US$ millions. Source: COMIGO/C Vale dataset. Action plan text is illustrative.</small>'
  },
  'CVALE':{
    channel:'CHANNEL · COOPERATIVE · MINDCOOP',
    title:'C Vale',logo:'C.VALE',logoSub:'COOPERATIVA AGROINDUSTRIAL',
    sellin:{label:'Sell-in trend & 2026 target (YTD realized)',years:['2023','2024','2025'],vals:[21.2,16.6,22.0],target:23,ytd:5,ytdPct:'22%',max:28,ticks:[0,10,20,28]},
    sellinMix:{label:'Sell-in mix by category (2025)',center:'$22M',centerSub:'2025',items:[
      {n:'Fungicides (10.5)',v:10.5,c:'#4A8FA0'},{n:'Herbicides (5.6)',v:5.6,c:'#1F4E5F'},
      {n:'Insecticides (4.7)',v:4.7,c:'#6B8E4E'},{n:'ST (0.6)',v:0.6,c:'#A8B84B'},
      {n:'Biologicals (0.5)',v:0.5,c:'#C9B27C'},{n:'Other (0.2)',v:0.2,c:'#555'}]},
    sellout:{label:'Sell-out — history, targets & 26/27 YTD',cats:['23/24','24/25','25/26','26/27 T','27/28 T','28/29 T'],vals:[31,20,24,29,34,44],histCount:3,ytdIdx:3,ytdVal:1,max:50,ticks:[0,15,30,45]},
    inventory:{pct:45,status:'ABOVE HEALTHY LINE',healthy:30,note:'Inv 11MUSD  |  Sell-out 25/26  24MUSD'},
    mix:{label:'Mix de produto — C.VALE 2025',targetLine:67,note:'Estratégico + Lançamento 41%',items:[
      {label:'Lançamento',v:5,c:'#C9A227'},{label:'Estratégico',v:36,c:'#4A8FA0'},
      {label:'Regular',v:20,c:'#6B8E4E'},{label:'Commodity',v:39,c:'#8B9BAD'}]},
    actionPlan:[
      '<b>Sales Tracking:</b> Continuous sales monitoring and route adjustments with C.Vale — macro tracking, quarterly strategic reviews, pulse follow-ups, and a weekly ranking (KAM/DSM, every 7–30 days).',
      '<b>Sales Enablement:</b> Team training and quarterly product-focused sales pushes (Ligar, Exalt, Paxeo, Elevore, Ulrisha, etc.), supported by a full-year technical plan aligned with agronomy (RCs/DSM/Agronomy).',
      '<b>Field Demos & Influencers:</b> Technical tours and influencer projects (Fundação MS, S2 Agro, Vanguarda) across summer and safrinha to prove results and engage key growers (Agronomy/KAM).'
    ],
    soc:{label:'SOC - Crop Protection',items:[
      {n:'Syngenta',v:34.5,c:'#4A8FA0'},{n:'UPL',v:25.5,c:'#C9A227'},
      {n:'Bayer',v:14.5,c:'#B85C4A'},{n:'Corteva',v:10.9,c:'#2A9D8F',hl:true},
      {n:'BASF',v:10.0,c:'#1F3B57'},{n:'Outros',v:4.5,c:'#6B8E3F'}],max:40},
    takeaway:'Sell-in 2025 US$22.0M (+32% vs 2024); sell-out 25/26 US$24.0M, targeted to US$43.7M by 28/29; inventory 45% of sell-out (>30%, above healthy line).<br>Challenge: 2026 sell-in target US$0.1M vs 2025 US$22.0M (-100%); 2026 YTD invoiced+orders US$4.7M.<br><small>Note: Values in US$ millions. Source: COMIGO/C Vale dataset. Action plan text is illustrative.</small>'
  },
  'INTEGRADA':{
    channel:'CHANNEL 11 OF 12 · COOPERATIVE · MINDCOOP',
    title:'Integrada',logo:'INTEGRADA',logoSub:'COOPERATIVA AGROINDUSTRIAL',
    sellin:{label:'Sell-in trend & 2026 target (YTD realized)',years:['2023','2024','2025'],vals:[7.4,7.3,10.4],target:13,ytd:6,ytdPct:'47%',max:16,ticks:[0,4,8,12,16]},
    sellinMix:{label:'Sell-in mix by category (2025)',center:'$10M',centerSub:'2025',items:[
      {n:'Herbicides (4.8)',v:4.8,c:'#1F4E5F'},{n:'Fungicides (3.2)',v:3.2,c:'#4A8FA0'},
      {n:'Insecticides (1.2)',v:1.2,c:'#6B8E4E'},{n:'ST (0.9)',v:0.9,c:'#A8B84B'},
      {n:'F&V, Coffee & Citrus (0.2)',v:0.2,c:'#C9B27C'},{n:'Other (0.0)',v:0.02,c:'#555'}]},
    ...
    soc:{label:'SOC - Crop Protection',items:[
      {n:'Outros',v:39.9,c:'#6B8E3F'},{n:'Syngenta',v:18.1,c:'#1F3B57'},
      {n:'UPL',v:14.0,c:'#C9A227'},{n:'Corteva',v:11.4,c:'#2A9D8F',hl:true},
      {n:'Bayer',v:8.7,c:'#B85C4A'},{n:'IHARA',v:7.8,c:'#3E7C6E'}],max:50},
  ...
  'COTRIJAL':{
    channel:'CHANNEL · COOPERATIVE · MINDCOOP',
    title:'Cotrijal',logo:'COTRIJAL',logoSub:'COOPERATIVA AGROPECUÁRIA',
    sellin:{label:'Sell-in trend & 2026 target (YTD realized)',years:['2023','2024','2025'],vals:[6.7,7.5,7.4],target:9,ytd:3.2,ytdPct:'36%',max:12,ticks:[0,4,8,12]},
    sellinMix:{label:'Sell-in mix by category (2025)',center:'$7.4M',centerSub:'2025',items:[
      {n:'Fungicides (4.4)',v:4.4,c:'#4A8FA0'},{n:'Herbicides (1.8)',v:1.8,c:'#1F4E5F'},
      {n:'Insecticides (0.8)',v:0.8,c:'#6B8E4E'},{n:'ST (0.4)',v:0.4,c:'#A8B84B'}]},
    sellout:{label:'Sell-out — history, targets & 26/27 YTD',cats:['23/24','24/25','25/26','26/27 T','27/28 T','28/29 T'],vals:[4,5,6,8,11,16],histCount:3,ytdIdx:3,ytdVal:0.4,max:20,ticks:[0,5,10,15,20]},
    inventory:{pct:27,status:'HEALTHY LINE',healthy:30,note:'Inventory healthy (27% of sell-out)'},
    actionPlan:[
      '<b>Service model and coverage:</b> successfully transition to the new geography-based service model focused on growers.',
      '<b>Fungicides Demand Generation:</b> Maintain the #1 position in fungicides (60% of client and Corteva business).',
      '<b>Drive growth in herbicides</b> and new products: Elevere, Gapper, Spider, Verdict Ultra, and Utrisha N.'
    ],
    soc:{label:'SOC - Crop Protection',items:[
      {n:'Outros',v:30.0,c:'#6B8E3F'},{n:'Syngenta',v:22.0,c:'#4A8FA0'},
      {n:'Corteva',v:20.0,c:'#2A9D8F',hl:true},{n:'UPL',v:14.0,c:'#C9A227'},
      {n:'Bayer',v:9.0,c:'#B85C4A'},{n:'IHARA',v:5.0,c:'#3E7C6E'}],max:35},
    takeaway:'Sell-in -2% in 2025 (+12% CAGR from 2023); sell-out targeted +93% to $16M by 28/29; inventory healthy (27% of sell-out).<br>Challenge: 2026 sell-in target $9M is +27% vs 2025 — stock must sell down first.'
  }
};
...
function opSocHTML(d){
  ...
  d.items.forEach(it=>{
    ...
    const rightLabel = d.ranking ? (it.place||'') : it.v.toFixed(1)+'%';
    h+='<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">'
      ...
      +'</div>'
      +'<div style="font-size:.65rem;font-weight:700;color:'+(it.hl?'#2ABFBF':'#CBD5E1')+';min-width:54px;text-align:left;line-height:1.2">'+rightLabel+'</div>'
      +'</div>';
  });
  return h;
}
function opMixHTML(d){
  if(!d||!d.items||!d.items.length) return '';
  // Gráfico de barras empilhadas com linha de target Corteva
  // items: [{label, v, c}] onde v é percentual
  const total = d.items.reduce((s,i)=>s+i.v, 0);
  const H = 200, W = 120, barW = 50, x0 = 55;
  const yPct = pct => H - (pct/100*H);
  
  // Construir segmentos empilhados (de baixo para cima)
  let segments = '';
  let cumPct = 0;
  // Inverter para empilhar de baixo: Commodity na base, depois Regular, Estratégico, Lançamento
  const items = [...d.items].reverse();
  items.forEach(it => {
    const y1 = yPct(cumPct);
    const y2 = yPct(cumPct + it.v);
    const h = y1 - y2;
    segments += `<rect x="${x0}" y="${y2.toFixed(1)}" width="${barW}" height="${h.toFixed(1)}" fill="${it.c}"/>`;
    // Label percentual dentro da barra
    if(h > 14){
      const cy = (y1+y2)/2;
      segments += `<text x="${x0+barW/2}" y="${cy+4}" font-size="11" font-weight="700" fill="#fff" text-anchor="middle">${it.v}%</text>`;
    }
    cumPct += it.v;
  });

  // Linha de target Corteva (ex: 67%)
  let targetLine = '';
  if(d.targetLine){
    const ty = yPct(d.targetLine);
    targetLine = `
      <line x1="${x0-8}" y1="${ty}" x2="${x0+barW+40}" y2="${ty}" stroke="#E05050" stroke-width="1.5" stroke-dasharray="4,3"/>
      <text x="${x0+barW+4}" y="${ty-3}" font-size="8.5" fill="#E05050" font-weight="700">Target Corteva ${d.targetLine}%</text>`;
    // Calcular gap = target - (Estratégico + Lançamento)
    const estratLanc = d.items.filter(i=>i.label==='Lançamento'||i.label==='Estratégico').reduce((s,i)=>s+i.v,0);
    const gap = d.targetLine - estratLanc;
    if(gap > 0){
      const yEstrat = yPct(estratLanc);
      const arrowMid = (ty + yEstrat) / 2;
      targetLine += `
        <line x1="${x0-4}" y1="${ty}" x2="${x0-4}" y2="${yEstrat}" stroke="#E05050" stroke-width="1.5"/>
        <text x="${x0-6}" y="${arrowMid+4}" font-size="8" fill="#E05050" text-anchor="end">gap</text>
        <text x="${x0-6}" y="${arrowMid+14}" font-size="8" fill="#E05050" text-anchor="end">${gap} p.p.</text>`;
    }
  }

  // Legenda
  const legendItems = [...d.items].reverse();
  let legend = legendItems.map((it,i)=>`
    <g transform="translate(0,${i*16})">
      <rect width="10" height="10" fill="${it.c}" rx="2"/>
      <text x="14" y="9" font-size="9" fill="#CBD5E1">${it.label}</text>
    </g>`).join('');

  // Nota rodapé
  const nota = d.note ? `<text x="${x0+barW/2}" y="${H+28}" font-size="8.5" fill="#2A9D8F" font-weight="700" text-anchor="middle">${d.note}</text>` : '';

  return `<div>
    <div style="font-size:.68rem;font-weight:700;color:var(--g300);margin-bottom:6px;text-align:center">${d.label||'Mix de produto'}</div>
    <svg viewBox="0 0 ${W+100} ${H+40}" style="width:100%;max-width:260px;height:auto;overflow:visible">
      <!-- Eixo Y -->
      <line x1="${x0}" y1="0" x2="${x0}" y2="${H}" stroke="rgba(255,255,255,.15)" stroke-width="1"/>
      <!-- Barras empilhadas -->
      ${segments}
      <!-- Target line -->
      ${targetLine}
      ...
      ${nota}
      <!-- Legenda -->
      <g transform="translate(${x0+barW+10},${H-legendItems.length*16})">${legend}</g>
    </svg>
  </div>`;
}
function renderOnePage(){
  ...
  let h='<div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:18px">'
    ...
    +'<div style="display:grid;grid-template-columns:'+(d.mix?'1fr 1fr 1fr 1fr':'1fr 1fr 1fr')+';gap:16px;margin-bottom:16px">'
      +(d.mix?'<div class="card" style="padding:18px">'+opMixHTML(d.mix)+'</div>':'')
...
function exportPDF(){
  ...
  // Usar print do navegador com CSS específico para impressão
  const title = document.getElementById('bc-text')?.textContent || 'ESCALA 2026';
  const origTitle = document.title;
  document.title = 'ESCALA 2026 - ' + title;
  // Adicionar classe para print
...
function renderPac(){
  ...
  if(!el) return;
  if(!D.pac) D.pac=[];
  // Garantir campos obrigatórios em cada coop
  D.pac.forEach(p=>{
    if(!p.plano) p.plano=[];
    if(!p.acoes) p.acoes=[];
    if(!p.dados) p.dados='';
    if(!p.pincKam) p.pincKam='';
    // Migrar campos antigos
    if(!p.dados && (p.soc||p.sellin||p.sellout||p.estoque)){
      p.dados = [p.soc?'SOC: '+p.soc:'', p.sellin?'Sell-in: '+p.sellin:'', p.sellout?'Sell-out: '+p.sellout:'', p.estoque?'Estoque: '+p.estoque:''].filter(Boolean).join('\n');
    }
  });
  ...
  el.innerHTML=D.pac.map((p,pi)=>{
    const planoRows=p.plano.map((ln,li)=>`
      <tr>
        <td contenteditable="true" onblur="D.pac[${pi}].plano[${li}].item=this.textContent;saveD(true)" style="padding:7px 10px;font-size:.76rem;color:var(--white);border-bottom:1px solid rgba(255,255,255,.05);word-break:break-word;max-width:0">${ln.item||''}</td>
        <td contenteditable="true" onblur="D.pac[${pi}].plano[${li}].resp=this.textContent;saveD(true)" style="padding:7px 10px;font-size:.76rem;color:var(--g200);border-bottom:1px solid rgba(255,255,255,.05);word-break:break-word;max-width:0">${ln.resp||''}</td>
        <td contenteditable="true" onblur="D.pac[${pi}].plano[${li}].prazo=this.textContent;saveD(true)" style="padding:7px 10px;font-size:.76rem;color:var(--g400);border-bottom:1px solid rgba(255,255,255,.05);word-break:break-word">${ln.prazo||''}</td>
    ...
    const acoesRows=p.acoes.map((a,ai)=>`
      <tr>
        <td contenteditable="true" onblur="D.pac[${pi}].acoes[${ai}].acao=this.textContent;saveD(true)" style="padding:7px 10px;font-size:.76rem;color:var(--white);border-bottom:1px solid rgba(255,255,255,.05);word-break:break-word;white-space:normal;vertical-align:top">${a.acao||''}</td>
        <td contenteditable="true" onblur="D.pac[${pi}].acoes[${ai}].resp=this.textContent;saveD(true)" style="padding:7px 10px;font-size:.76rem;color:var(--g200);border-bottom:1px solid rgba(255,255,255,.05);word-break:break-word;white-space:normal;vertical-align:top">${a.resp||''}</td>
        <td contenteditable="true" onblur="D.pac[${pi}].acoes[${ai}].prazo=this.textContent;saveD(true)" style="padding:7px 10px;font-size:.76rem;color:var(--g400);border-bottom:1px solid rgba(255,255,255,.05);word-break:break-word;white-space:normal;vertical-align:top">${a.prazo||''}</td>
    ...
    return `
    <div class="card" style="margin-bottom:20px;padding:0;overflow:hidden;overflow-x:auto">
      <!-- Cabeçalho -->
      <div style="background:var(--navy-m);padding:14px 18px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px">
        ...
        <button class="btn btn-g btn-sm" style="background:rgba(224,80,80,.2);border:1px solid #E05050;color:#E05050" onclick="D.pac.splice(${pi},1);renderPac();saveD(true)" title="Remover ${p.coop} do PAC">🗑 Remover</button>
      </div>
      <!-- One Page -->
      <div style="padding:18px;border-bottom:1px solid rgba(255,255,255,.08)">
        <div style="font-size:.72rem;font-weight:800;color:var(--teal);letter-spacing:.8px;text-transform:uppercase;margin-bottom:14px;padding-bottom:6px;border-bottom:1px solid rgba(42,191,191,.2)">📄 One Page — ${p.coop}</div>
        ${(()=>{
          const od=ONEPAGE_DATA[p.coop.toUpperCase()];
          if(!od) return '<div style="color:var(--g600);font-size:.8rem;padding:12px;background:rgba(255,255,255,.03);border-radius:8px;text-align:center">One Page não cadastrado para esta cooperativa</div>';
          return '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:14px">'
            +'<div class="card" style="padding:14px;background:var(--navy-d)">'
              +'<div style="font-size:.62rem;font-weight:800;color:var(--teal);letter-spacing:1px;margin-bottom:8px;padding-bottom:5px;border-bottom:1px solid rgba(42,191,191,.18)">SELL-IN</div>'
              +'<div style="font-size:.7rem;color:var(--g400);margin-bottom:6px">'+od.sellin.label+'</div>'
              +opLineChartHTML(od.sellin)+'</div>'
            +'<div class="card" style="padding:14px;background:var(--navy-d)">'
              +'<div style="font-size:.62rem;font-weight:800;color:var(--teal);letter-spacing:1px;margin-bottom:8px;padding-bottom:5px;border-bottom:1px solid rgba(42,191,191,.18)">SELL-IN MIX</div>'
              +opDonutHTML(od.sellinMix)+'</div>'
            +'<div class="card" style="padding:14px;background:var(--navy-d)">'
              +'<div style="font-size:.62rem;font-weight:800;color:var(--teal);letter-spacing:1px;margin-bottom:8px;padding-bottom:5px;border-bottom:1px solid rgba(42,191,191,.18)">SELL-OUT</div>'
              +opBarChartHTML(od.sellout)+'</div>'
          +'</div>'
          +'<div style="display:grid;grid-template-columns:'+(od.mix?'1fr 1fr 1fr 1fr':'1fr 1fr 1fr')+';gap:14px">'
            +(od.mix?'<div class="card" style="padding:14px;background:var(--navy-d)">'+opMixHTML(od.mix)+'</div>':'')
            +'<div class="card" style="padding:14px;background:var(--navy-d);text-align:center">'
              +'<div style="font-size:.62rem;font-weight:800;color:var(--teal);letter-spacing:1px;margin-bottom:8px;padding-bottom:5px;border-bottom:1px solid rgba(42,191,191,.18);text-align:left">INVENTORY</div>'
              +'<div style="display:flex;justify-content:center">'+opGaugeHTML(od.inventory)+'</div>'
              +'<div class="tag '+(od.inventory.pct>30?'tag-red':'tag-green')+'" style="display:inline-flex;margin-top:6px;font-size:.65rem">'+od.inventory.status+'</div>'
              +'<div style="font-size:.62rem;color:var(--g400);margin-top:5px">'+od.inventory.note+'</div>'
            +'</div>'
            +'<div class="card" style="padding:14px;background:var(--navy-d)">'
              +'<div style="font-size:.62rem;font-weight:800;color:var(--teal);letter-spacing:1px;margin-bottom:8px;padding-bottom:5px;border-bottom:1px solid rgba(42,191,191,.18)">ACTION PLAN</div>'
              +od.actionPlan.map((a,ai)=>'<div style="display:flex;gap:8px;'+(ai?'margin-top:10px;padding-top:10px;border-top:1px solid rgba(255,255,255,.06)':'')+'">'
                +'<span style="width:7px;height:7px;background:var(--teal);border-radius:2px;flex-shrink:0;margin-top:4px"></span>'
                +'<div style="font-size:.7rem;color:var(--g200);line-height:1.55">'+a+'</div></div>').join('')
            +'</div>'
            +'<div class="card" style="padding:14px;background:var(--navy-d)">'+opSocHTML(od.soc)+'</div>'
          +'</div>'
          +'<div class="card" style="border-left:4px solid var(--teal);padding:12px 16px;margin-top:14px;background:var(--navy-d)">'
            +'<div style="font-size:.6rem;font-weight:800;color:var(--amber);letter-spacing:1px;margin-bottom:5px">KEY TAKEAWAY</div>'
            +'<div style="font-size:.75rem;color:var(--g200);line-height:1.65">'+od.takeaway+'</div>'
          +'</div>';
        })()}
      </div>
      <!-- PARTE 1: Dados -->
      <div style="padding:18px">
        <div style="margin-bottom:18px">
          <div style="font-size:.72rem;font-weight:800;color:var(--teal);letter-spacing:.8px;text-transform:uppercase;margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid rgba(42,191,191,.2)">📊 Parte 1 — Dados da Coop</div>
          <div contenteditable="true" onblur="D.pac[${pi}].dados=this.textContent;saveD(true)" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:8px;padding:12px 14px;font-size:.82rem;color:var(--white);min-height:80px;outline:none;word-break:break-word;white-space:pre-wrap;line-height:1.7">${p.dados||''}</div>
        </div>
        <!-- PARTE 2: PINC KAM -->
        <div style="margin-bottom:18px">
          <div style="font-size:.72rem;font-weight:800;color:var(--teal);letter-spacing:.8px;text-transform:uppercase;margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid rgba(42,191,191,.2)">📋 Parte 2 — PINC KAM</div>
          <div contenteditable="true" onblur="D.pac[${pi}].pincKam=this.textContent;saveD(true)" style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:8px;padding:12px 14px;font-size:.82rem;color:var(--white);min-height:80px;outline:none;word-break:break-word;white-space:pre-wrap;line-height:1.7">${p.pincKam||''}</div>
        </div>
        <!-- PARTE 3: Ações -->
        <div>
          ...
          <table style="width:100%;border-collapse:collapse;table-layout:fixed">
            <thead><tr style="background:rgba(255,255,255,.05)">
              <th style="padding:6px 10px;font-size:.65rem;color:var(--g400);text-align:left;width:38%">Ação</th>
              <th style="padding:6px 10px;font-size:.65rem;color:var(--g400);text-align:left;width:24%">Resp.</th>
              <th style="padding:6px 10px;font-size:.65rem;color:var(--g400);text-align:left;width:14%">Prazo</th>
              <th style="padding:6px 10px;font-size:.65rem;color:var(--g400);text-align:left;width:12%">Status</th>
...
let _origPageHTMLs = null;
let _origNavHTML = null;
let _origTopbarHTML = null;
...
async function _toggleTranslate(){
  ...
  if(!_isEnglish){
    ...
    // Guardar HTML original de TODAS as páginas e nav
    _origPageHTMLs = {};
    document.querySelectorAll('.page').forEach(p=>{ _origPageHTMLs[p.id]=p.innerHTML; });
    _origNavHTML = document.querySelector('nav')?.innerHTML||'';

    // Coletar todos os nós de texto únicos
    const textMap = new Map(); // texto → [nós]
    // Padrões que NÃO traduzir
    const skipRe = /^[\d\s\-\+\.,%$R€\/:@\(\)]+$|^[A-ZÁÉÍÓÚÂÊÎÔÛÃÕÇ]{2,}(\s[A-ZÁÉÍÓÚÂÊÎÔÛÃÕÇ]{2,})+$|^\s*$/;
    const keepNames = new Set(['MINDCOOP','INFOCOOP','INTECOOP','ELITECOOP','GOCOOP',
      'ESCALA','CORTEVA','ZMP','KAM','RSL','RML','DSM','NPS','SOC','PINC','PAC','RSVP',
      'LAR','COAMO','CVALE','COCAMAR','COMIGO','COPACOL','COOPERALFA','CAMDA','COPLACANA',
      'COPERCANA','CAPAL','COTRIJAL','INTEGRADA','COOPERMOTA','COOPERCITRUS','C.VALE',
      'TRADIÇÃO','COTRIEL','COTRIJUC','COTRICAMPO','COTRISAL','COTRISUL','COOXUPÉ',
      'COPLANA','COPASUL','COPAGRIL','COOPAVEL','HOLAMBRA','COTRIBÁ','AGRÁRIA','FRÍSIA',
      'CASTROLANDA','BOM JESUS','COASUL','COAGRIL','COOPATRIGO','COCARI','COPERCAMPOS',
      'COPÉRDIA','A1','COOPAGRÍCOLA','COOPERTRADIÇÃO']);

    document.querySelectorAll('.page, nav').forEach(section=>{
      const walker = document.createTreeWalker(section, NodeFilter.SHOW_TEXT, {
        acceptNode: n => {
          const p = n.parentElement;
          if(!p) return NodeFilter.FILTER_REJECT;
          if(['SCRIPT','STYLE','OPTION','INPUT','TEXTAREA','SELECT'].includes(p.tagName)) return NodeFilter.FILTER_REJECT;
          const t = n.textContent.trim();
          if(!t || t.length < 2 || t.length > 500) return NodeFilter.FILTER_SKIP;
          if(keepNames.has(t.toUpperCase())) return NodeFilter.FILTER_SKIP;
          if(skipRe.test(t)) return NodeFilter.FILTER_SKIP;
    ...
    const allTexts = [...textMap.keys()];
    const BATCH = 50;
    const translations = {};

    // Traduzir em lotes via API Claude
      ...
      try{
        const resp = await fetch('https://api.anthropic.com/v1/messages',{
          ...
          body:JSON.stringify({
            ...
            max_tokens:2000,
            system:'Professional translator PT→EN for executive dashboards. Return ONLY valid JSON mapping each original text to English. Rules: preserve cooperative names (COTRIJAL, COAMO, LAR etc), person names (all caps full names), program names (MINDCOOP, GOCOOP etc), acronyms (KAM, RSL, SOC, NPS), numbers, dates, currencies, emails, phone numbers. Translate everything else to formal business English.',
            messages:[{role:'user',content:'Translate each text. Return JSON object original to translation: '+JSON.stringify(batch)}]
          })
        });
        if(resp.ok){
          ...
          try{ Object.assign(translations, JSON.parse(clean)); }catch(e){}
        }
      }catch(e){ console.warn('batch',i,e); }
    }

    // Aplicar traduções
    textMap.forEach((nodes,orig)=>{
      const tr = translations[orig];
      if(tr && tr!==orig){
        nodes.forEach(n=>{
          n.textContent = n.textContent.replace(orig,tr);
        });
      }
    });

    _isEnglish=true;
    btn.textContent='PT'; btn.disabled=false; btn.style.color='#F5A623';

  } else {
    // Restaurar português
    if(_origPageHTMLs){
      document.querySelectorAll('.page').forEach(p=>{
        if(_origPageHTMLs[p.id]) p.innerHTML=_origPageHTMLs[p.id];
      });
    }
    if(_origNavHTML){ const nav=document.querySelector('nav'); if(nav) nav.innerHTML=_origNavHTML; }
    document.querySelectorAll('.ni').forEach(n=>{
      n.addEventListener('click',function(){navigate(this.dataset.page);});
    });
    _isEnglish=false;
    btn.textContent='EN'; btn.style.color='';
  }
}

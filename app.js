...
async function _toggleTranslate(){
  ...
  if(!_isEnglish){
    ...
    // Nomes que NUNCA traduzir
    const noTranslate = new Set([
      'MINDCOOP','INFOCOOP','INTECOOP','ELITECOOP','GOCOOP','ESCALA',
      'CORTEVA','ZMP','KAM','RSL','RML','DSM','NPS','SOC','PINC','PAC',
      'RSVP','PDF','PT','EN','LAR','COAMO','CVALE','COCAMAR','COMIGO',
      'COPACOL','COOPERALFA','CAMDA','COPLACANA','COPERCANA','CAPAL',
      'COTRIJAL','INTEGRADA','COOPERMOTA','COOPERCITRUS','A1','AGRARIA',
      'CASTROLANDA','FRISIA','BOM JESUS','COTRISAL','COTRISUL','COOXUPE',
      'COPLANA','COPASUL','COPAGRIL','COOPAVEL','COOPERTRADIÇÃO','COTRIEL',
      'COTRIJUC','COTRICAMPO','COTRIBÁ','HOLAMBRA','COPÉRDIA','COPERCAMPOS',
      'COASUL','COAGRIL','COOPATRIGO','COCARI','COOPAGRÍCOLA',
    ]);

    // Coletar todos os nós de texto únicos do cockpit
    const textMap = new Map();
    const skipRe = /^[\d\s\-\+\.,%$R€\/\:]+$|@|^\s*$|^[A-Z]{2,}(\s[A-Z]{2,})+$/;
    ...
    document.querySelectorAll('.page, nav').forEach(section=>{
      const walker = document.createTreeWalker(section, NodeFilter.SHOW_TEXT, {
        acceptNode: n => {
          ...
          if(!t || t.length < 2 || t.length > 400) return NodeFilter.FILTER_SKIP;
          if(noTranslate.has(t.toUpperCase())) return NodeFilter.FILTER_SKIP;
      ...
      try{
        const resp = await fetch('https://api.anthropic.com/v1/messages',{
          ...
          body:JSON.stringify({
            ...
            max_tokens:2000,
            system:'You are a professional translator for executive business dashboards. Translate Brazilian Portuguese to formal business English. Return ONLY a valid JSON object with no markdown. Preserve: proper nouns (cooperative names, person names), program names (MINDCOOP, ESCALA, GOCOOP etc), acronyms (KAM, RSL, SOC etc), numbers, dates, currencies, percentages.',
            messages:[{role:'user',content:'Translate each text in this JSON array and return a JSON object mapping original→translation:\n'+JSON.stringify(batch)}]
          })
        });
        if(resp.ok){
          ...
          try{ Object.assign(translations, JSON.parse(clean)); }catch(e){ console.warn('parse err',e,clean.slice(0,200)); }
        }
      }catch(e){ console.warn('Translation batch error',i,e); }
    }
    ...
    textMap.forEach((nodes,orig)=>{
      ...
      if(tr && tr!==orig){
        nodes.forEach(n=>{
          if(n.textContent.trim()===orig) n.textContent=n.textContent.replace(orig,tr);
        });
      }
    });

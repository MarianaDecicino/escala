...
async function _toggleTranslate(){
  ...
  const ptToEn = {
    ...
    'Mapeamento de Temas':'Theme Mapping','Ritual':'Ritual',
    'Próximos Passos e Responsáveis':'Next Steps & Owners',
    ...
    'Objetivo do Encontro':'Meeting Objective','Fases':'Phases',
    'Principal':'Main','Gestão':'Management','Encontros':'Meetings','Dados':'Data',
    'Visão geral':'Overview','Programas do ESCALA':'ESCALA Programs',
    'Exportar PDF':'Export PDF','Salvar':'Save','Alterar senha':'Change password',
    'Sair':'Log out','Últimos Destaques':'Latest Highlights',
    'Informações Relevantes — Destaques':'Relevant Information — Highlights',
    'Todas':'All','Cooperativa':'Cooperative','Radar':'Radar','Detalhe':'Detail',
    'Rel. ZMP':'Rel. ZMP','KAM':'KAM','No plano':'On track','Em atenção':'Attention needed',
    'Crítico':'Critical','Concluído':'Completed','Em prep.':'Preparing',
    'Atrasado':'Delayed','Em progresso':'In progress','Não iniciado':'Not started',
    'Plano':'Plan','Ações':'Actions','Parte 1 — Dados da Coop':'Part 1 — Coop Data',
    'Parte 2 — Plano':'Part 2 — Plan','Parte 3 — Ações':'Part 3 — Actions',
    'PAC — Plano de Aceleração das Cooperativas':'PAC — Cooperative Acceleration Plan',
    'Relacionamento Coops':'Coops Relationship','Timeline':'Timeline',
    'Última atualização':'Last update','Atualizado hoje':'Updated today',
    'Item':'Item','Resp.':'Owner','Ação':'Action','Data':'Date',
    'Sell-in mix by category':'Sell-in mix by category',
    'ACTION PLAN':'ACTION PLAN','KEY TAKEAWAY':'KEY TAKEAWAY',
    'SELL-IN':'SELL-IN','SELL-OUT':'SELL-OUT','INVENTORY':'INVENTORY',
    'Administração de Acessos':'Access Administration',
    'One Page Coops':'One Page Coops',
  };

  if(!_isEnglish){
    btn.textContent='⏳'; btn.disabled=true;
    // Guardar HTMLs originais de TODAS as páginas
    _origPageHTMLs = {};
    document.querySelectorAll('.page').forEach(p=>{
      _origPageHTMLs[p.id] = p.innerHTML;
    });
    // Guardar menu e labels
    _origNavHTML = document.querySelector('nav')?.innerHTML||'';
    _origTopbarHTML = document.getElementById('topbar')?.innerHTML||'';

    // Traduzir todos os textos em todas as páginas
    document.querySelectorAll('.page, nav, header').forEach(section=>{
      ...
      nodes.forEach(n=>{
        ...
        if(ptToEn[t]) n.textContent = n.textContent.replace(t, ptToEn[t]);
        else {
          // Tentar tradução parcial de labels no menu
          Object.entries(ptToEn).forEach(([pt,en])=>{
            if(n.textContent===pt) n.textContent=en;
          });
        }
      });
    });
    ...
    if(_origNavHTML) document.querySelector('nav').innerHTML=_origNavHTML;
    // Re-bind nav items
    document.querySelectorAll('.ni').forEach(n=>{
      n.addEventListener('click',function(){navigate(this.dataset.page);});
    });

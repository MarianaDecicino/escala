...
const ONEPAGE_DATA={
  'COAMO':{
    ...
    soc:{label:'SOC - Crop Protection (Ranking by Position)',items:[
      {n:'Bayer',v:80,c:'#B85C4A'},{n:'BASF',v:68,c:'#1F3B57'},
      {n:'Syngenta',v:55,c:'#4A8FA0'},{n:'UPL',v:40,c:'#C9A227'},
      {n:'Corteva',v:28,c:'#2A9D8F',hl:true}],max:90,ranking:true},
    takeaway:'Sell-in +17% in 2025 (-27% CAGR from 2023); sell-out targeted +131% to $85M by 28/29; inventory high (49% of sell-out, >30% healthy line).<br>Challenge: 2026 sell-in target $34M is +40% vs 2025 — stock must sell down first while stock is high.<br><i>Note: Inventory in april/26 was~ 11 MUSD.</i>'
  },
  'COPLACANA':{
  ...
  'LAR':{
    ...
    mix:{label:'Mix de produto — LAR 2025',targetLine:67,note:'Estratégico + Lançamento 44%',items:[
      {label:'Lançamento',v:11,c:'#C9A227'},{label:'Estratégico',v:10,c:'#4A8FA0'},
      {label:'Regular',v:34,c:'#6B8E4E'},{label:'Commodity',v:45,c:'#8B9BAD'}]},
    actionPlan:[
    ...
    takeaway:'Sell-in +94% in 2025 (+4% CAGR from 2023); sell-out targeted +230% to $52M by 28/29; inventory high (129% of sell-out, >30% healthy line).<br>Challenge: 2026 sell-in target $21M is -16% vs 2025 — a reset to clear inventory while stock is high.<br><i>Note: Inventory in april/26 was~ 19 MUSD and we had 1.2 MUSD of returns.</i>'
  },
  'COOPERCITRUS':{
    channel:'CHANNEL · COOPERATIVE · MINDCOOP',
    ...
    sellinMix:{label:'Sell-in mix by category (2025)',center:'$26M',centerSub:'2025',items:[
      {n:'Sugarcane (16.0)',v:16.0,c:'#1F4E5F'},{n:'F&V, Coffee & Citrus (3)',v:3,c:'#4A8FA0'},
    ...
    mix:{label:'Mix de produto — COOPERCITRUS 2025',targetLine:67,note:'Estratégico + Lançamento 74%',items:[
      {label:'Lançamento',v:8,c:'#C9A227'},{label:'Estratégico',v:18,c:'#4A8FA0'},
      {label:'Regular',v:21,c:'#6B8E4E'},{label:'Commodity',v:53,c:'#8B9BAD'}]},
    actionPlan:[
  ...
  'COMIGO':{
    ...
    actionPlan:[
      ...
      '<b>Marketing, Inventory Management & Governance:</b> Strengthen brand and campaigns; optimize stock through DSM (prioritizing near-expiry and high-demand products); and maintain monthly governance forums to track KPIs (led by Luiz Oliveira, Wheverton Cabral, Carlos Teoro, and Carlos Melo).'
    ],
    soc:{label:'SOC - Crop Protection (Ranking by Position)',items:[
      {n:'Player 1',v:80,c:'#6B8E3F'},{n:'Corteva',v:65,c:'#2A9D8F',hl:true},
      {n:'Player 3',v:45,c:'#1F3B57'},{n:'Player 4',v:30,c:'#C9A227'},
      {n:'Player 5',v:20,c:'#B85C4A'}],max:90,ranking:true},
    takeaway:'Sell-in 2025 US$23.8M (-24% vs 2024); sell-out 25/26 US$19.8M, targeted to US$32.0M by 28/29; inventory 255% of sell-out (>30%, above healthy line).<br>Challenge: 2026 sell-in target US$20.2M vs 2025 US$23.8M (-15%); 2026 YTD invoiced+orders US$13.5M.<br><i>Note: Values in US$ millions. Source: COMIGO/C Vale dataset. Action plan text is illustrative.</i>'
  },
  'CVALE':{
    channel:'CHANNEL · COOPERATIVE · MINDCOOP',
    ...
    mix:{label:'Mix de produto — C.VALE 2025',targetLine:67,note:'Estratégico + Lançamento 41%',items:[
      {label:'Lançamento',v:5,c:'#C9A227'},{label:'Estratégico',v:21,c:'#4A8FA0'},
      {label:'Regular',v:20,c:'#6B8E4E'},{label:'Commodity',v:53,c:'#8B9BAD'}]},
    actionPlan:[
    ...
    takeaway:'Sell-in 2025 US$22.0M (+32% vs 2024); sell-out 25/26 US$24.0M, targeted to US$43.7M by 28/29; inventory 45% of sell-out (>30%, above healthy line).<br>Challenge: 2026 sell-in target US$0.1M vs 2025 US$22.0M (-100%); 2026 YTD invoiced+orders US$4.7M.<br><i>Note: Values in US$ millions. Source: COMIGO/C Vale dataset. Action plan text is illustrative.</i>'
  },
  'INTEGRADA':{
...
function opSocHTML(d){
  ...
  const isRanking = d.ranking;
  d.items.forEach(it=>{
    const w = isRanking ? (it.v/d.max*100).toFixed(1) : (it.v/d.max*100).toFixed(1);
    const label = isRanking ? '' : it.v.toFixed(1)+'%';
    h+='<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">'
      ...
      +'</div>'
      +'<div style="font-size:.68rem;font-weight:700;color:'+(it.hl?'#2ABFBF':'#CBD5E1')+';width:54px;text-align:left">'+(isRanking?(it.n.includes('place')?'':it.v+'st place'||''):label)+'</div>'
      +'</div>';
  });
  return h;
}
